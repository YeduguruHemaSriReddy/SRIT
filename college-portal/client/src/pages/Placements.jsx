    ...
    <div className="min-h-screen bg-blue-50">
      ...
      <section className="pt-36 pb-28 text-center">
        ...
        <p className="text-xl text-gray-700 max-w-4xl mx-auto">
          ...
          through continuous skill development and top-company recruitment drives.
        </p>
      </section>
          ...
          {stats.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className={`rounded-3xl p-8 shadow-lg bg-gradient-to-br ${item.color} hover:from-indigo-500 hover:to-indigo-700 hover:text-white transition-all`}
              >
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {branches.map((b, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-blue-100 p-8 rounded-2xl shadow-md text-center hover:bg-indigo-600 hover:text-white transition-all"
              >
          ...
          <div className="grid md:grid-cols-2 gap-10">
            {features.map((f, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="flex gap-4 bg-blue-100 p-6 rounded-2xl shadow-md hover:bg-indigo-600 hover:text-white transition-all"
              >
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {recruiters.map((r, i) => (
              <motion.div
                ...
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true }}
                className="px-6 py-3 rounded-full bg-blue-100 shadow-md font-semibold hover:bg-indigo-600 hover:text-white transition-all"
              >
                {r}
              </motion.div>
            ))}
      ...
      <section className="py-28 text-center">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <p className="text-lg mb-10 text-slate-700">
            SRIT’s placement ecosystem ensures every student gets industry exposure.
          </p>

          <a href="tel:+919876543210">
            <Button
              size="lg"
              className="px-12 py-6 text-lg bg-blue-100 hover:bg-indigo-600 hover:text-white transition-all"
            >
              Contact Placement Cell ☎️
            </Button>
          </a>
        </div>
      </section>
